...
RSpec.describe 'Groups', type: :request do
  ...
  context 'DELETE /groups/:id/user/:user_id' do
    let!(:group) { create(:group, user_ids: user.id) }
    let!(:invited_user) { create(:user, :with_group_invitation, invitation_group_id: group.id ) }

    context 'when the pending user exists' do
      before :each do
        sign_in user

        delete "/groups/#{group.id}/users/#{invited_user.id}"
      ...
      it { expect(response).to redirect_to(group_path(group.id)) }
      it { expect(User.find(invited_user.id)).to eq(nil) }
    end

    context 'when the pending user does not exist' do
      before :each do
        sign_in user

        delete "/groups/#{group.id}/users/42"
